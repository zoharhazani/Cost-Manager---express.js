...
function validateDay(currDay){
    if (!currDay) {
        throw new Error('Day is not provided');
    }
    if (currDay > 31) {
...
router.post('/', async (req, res) => {
    try {
        ...
        generateCategory(req.body.category)
        ...
        let report = await Report.findOne({user_id: req.body.user_id, year: req.body.year, month: req.body.month});

        if (!report) {
            report = new Report({user_id: req.body.user_id, year: req.body.year, month: req.body.month, categories: {}});
        }
        ...
        report.categories[category].push({day: req.body.day, description: req.body.description, sum: req.body.sum});
